...
extends GenericSolver[S, Event]{
  ...
  private def nextFireEvent(config: Config, t: Int, i: Int, t1: Int)
      : FireOthersThen = {
    // Next thread to fire
    val t2 = 
  ...
  /** For each thread t, the results that are allowed for the return in 
    * position maxReachedFor(t).  This is used in the debug output. */
  ...
  def solve(events: Array[Event]) : Int = {
	...
	case Solve(config,i) => {
	  ...
	  events(i) match{
	    ...
	    case ReturnEvent(t, result) => {
	      ...
	      if(verbose) println(s"${(config,i)}: $t returns: $result")
	...
	case FireOthersThen(config, t, i, t1) => { 
	  ...
	  if(config.hasPending(t1) || t==t1){
		...
		if(verbose) println(s"$i: Fired $t1: $seqObj")
